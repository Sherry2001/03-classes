...
lengthProgram' :: IO Int
lengthProgram' = (\x -> putStrLn ("The length of the list is " ++ show x) >> return x) (length [1, 2, 3, 4, 5, 6])

-- let x = length [1,2,3,4,5,6] in putStrLn $ "The length of the list is " ++ show x

-- lengthProgram'' :: IO Int
-- lengthProgram'' = return (length [1, 2, 3, 4, 5, 6]) >>= (\x -> (putStrLn $ "The length of the list is" ++ show x) >> return x)
...
anotherProgram' =
  putStrLn "What is your name?"
    >> (getLine >>= \inpStr -> if inpStr == "Haskell" then putStrLn "you rock!" else putStrLn $ "Hello " ++ inpStr)
    >> putStrLn "That's all!"
